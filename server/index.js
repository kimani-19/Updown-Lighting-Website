...
// Load env from either `server/.env` (recommended for local dev)
// or from the repository root `.env` (common for process managers / hosting).
...
app.get('/api/reviews', async (req, res) => {
  ...
  console.log(`Attempting to fetch reviews. PLACE_ID loaded: ${!!placeId}, API_KEY loaded: ${!!apiKey}`);

  const url = `https://maps.googleapis.com/maps/api/place/details/json?place_id=${placeId}&fields=name,rating,reviews&key=${apiKey}`;

  if (!apiKey || !placeId) {
    console.error('Server configuration error: API_KEY or PLACE_ID is missing.');
  ...
  try {
    ...
    if (response.data.result && response.data.result.reviews) {
      ...
      const errorMessage = `Invalid response structure from Google API: ${JSON.stringify(response.data)}`;
      console.error('Unexpected response from Google Places API:', response.data);
      res.status(500).json({ error: 'Failed to process reviews from Google.', details: errorMessage });
    }
  } catch (error) {
